            ...
            if t_ind < len_sched - 1:
                ...
                if self.blsec_t.occ_ind == 0 and len(self.blsec_t.blsec_queue) == 0 and (not check_if_auto) and (ready_to_dept is True):
                    ...
                    self.blsec_t.process_autoblock_departure(
                        t, t_ind, next_event_tr_id, next_event_type, next_event_conn,
                        self.tr_next_event, self.stns_event[0], self.stns_event[1],
                        self.sched_act, self.total_schedule, self.headway_distance,
                        self.stn_line_occ_name, self.autoblock_stations,
                        self.tr_sched_updt, self.sched_updt,
                    )
